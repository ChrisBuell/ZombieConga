...
class GameScene: SKScene {
    ...
    let zombieMovesPerSecond: CGFloat = 480.0
...
//         addChild(background)
       ...
       // catsLabel.position = CGPoint.zero
...
//            if diff.length() <= zombieMovesPerSecond * CGFloat(dt){
...
//            }else{
    ...
    func moveZombieToward(location: CGPoint) {
        ...
        velocity = direction * zombieMovesPerSecond
    }
    ...
    func boundsCheckZombie() {
        let bottomLeft = CGPoint(x: cameraRect.minX, y: cameraRect.minY)//CGPoint.zero
        let topRight = CGPoint(x: cameraRect.maxX, y: cameraRect.maxY)//CGPoint(x: size.width, y: size.height)
...
//        enemy.position = CGPoint(x: size.width + enemy.size.width/2, y: CGFloat.random(min: playableRect.minY + enemy.size.height/2, max: playableRect.maxY - enemy.size.height/2))
       ...
       //duration originally 2.0
        let actionMove = SKAction.moveBy(x: -(size.width/2 + enemy.size.width),y:0, duration: 2.0)
    ...
    //change collision behavior here
    func zombieHit(cat: SKSpriteNode){
        ...
        let zCatColorChange = SKAction.colorize(with: SKColor.green, colorBlendFactor: 1.0, duration: 0.2)
        
        cat.run(zCatColorChange)
    ...
    func zombieHit(enemy: SKSpriteNode) {
        ...
        let blinkAction = SKAction.customAction(withDuration: duration) {
            node, elapsedTime in let slice = duration / blinkTimes
    ...
    func checkCollisions() {
        ...
        //This was missing when it wasn't working
        ...
        enumerateChildNodes(withName: "enemy") {node, _ in
            ...
            if node.frame.insetBy(dx: 20, dy: 20).intersects(self.zombie.frame) {hitEnemies.append(enemy)
            }
            }
    ...
    func moveTrain() {
        ...
        catsLabel.text = "Number of Cats: \(trainCount)"
